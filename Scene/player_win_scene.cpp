...
void Player_Win_Scene::Initialize(){
    ...
    AddNewObject(new Engine::Label("<< 玩家勝利 >>", "font2.ttc", 150, halfW, 150, 223, 21, 21, 255, 0.5, 0.5));
    ...
    btn->SetOnClickCallback(std::bind(&Player_Win_Scene::BackOnClick, this, 1));
    ...
    btn->SetOnClickCallback(std::bind(&Player_Win_Scene::BackOnClick, this, 2));
    AddNewControlObject(btn);
...
void Player_Win_Scene::Terminate(){
    AudioHelper::StopBGM(bgmId);
    IScene::Terminate();

}
void Player_Win_Scene::BackOnClick(int stage){
    if(stage==1)
